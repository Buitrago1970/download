...
def resolve_text(text: str) -> MediaMeta:
    info = _youtube_info(text, search=True)
    artist = info.get("artist") or info.get("uploader") or info.get("channel")
...
def _find_generated_mp3(tmpdir: str, fallback_id: Optional[str]) -> Optional[str]:
    if fallback_id:
        by_id = os.path.join(tmpdir, f"{fallback_id}.mp3")
...
def _download_raw_mp3(meta: MediaMeta, tmpdir: str) -> str:
    ...
    for fmt in formats:
        ...
        try:
            ...
            mp3_path = _find_generated_mp3(tmpdir, source_id)
            if mp3_path:
                return mp3_path
...
def download_from_input(value: str) -> tuple[str, MediaMeta]:
    meta = resolve_input(value)

    with tempfile.TemporaryDirectory() as tmpdir:
        mp3_path = _download_raw_mp3(meta, tmpdir)
        _embed_metadata(mp3_path, meta)

        final_name = _safe_filename(meta) + ".mp3"
        final_path = os.path.join(tmpdir, final_name)
        os.rename(mp3_path, final_path)

        with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as stable:
            with open(final_path, "rb") as f:
                shutil.copyfileobj(f, stable)
            stable_path = stable.name

    return stable_path, meta
...
def _run_playlist_job(job_id: str, playlist_url: str):
    ...
    try:
        ...
        for idx, track in enumerate(tracks, start=1):
            ...
            try:
                with tempfile.TemporaryDirectory() as tmpdir:
                    mp3_path = _download_raw_mp3(track, tmpdir)
                    _embed_metadata(mp3_path, track)
                    output_name = f"{idx:03d} - {_safe_filename(track)}.mp3"
...
async def download(payload: dict, background_tasks: BackgroundTasks):
    ...
    file_path, meta = download_from_input(value)
    filename = _safe_filename(meta) + ".mp3"
...
async def playlist_start(payload: dict):
    ...
    thread = threading.Thread(target=_run_playlist_job, args=(job_id, str(value)), daemon=True)
    thread.start()
...
async def playlist_file_download(job_id: str, file_id: str):
    ...
    return FileResponse(file_path, filename=file_entry["filename"], media_type="audio/mpeg")
